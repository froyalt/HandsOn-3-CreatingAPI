...
const dashboardController = {
  ...
  numPsicologo: async (req, res) => {
    try {
      ...
      res.status(200).json(`No momento existem ${numPsicologos.length} psicólogos cadastrados.`)
    } catch (error) {
  ...
  numPaciente: async (req, res) => {
    try {
      ...
      res.status(200).json(`No momento existem ${numPacientes.length} pacientes cadastrados.`)
    } catch (error) {
      res.status(400).json({ error });
    }
  },
  ...
  numAtendimento: async (req, res) => {
    try {
      ...
      res.status(200).json(`Foram feitos ${numAtendimentos.length} atendimentos no total.`)
    } catch (error) {
      res.status(400).json({ error });
    }
  },
